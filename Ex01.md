...
void reset(int ar[], int size) {
    int i;
    // reset all the values of the mapping array to zero
    for (i = 0; i < size; i++) {
        ar[i] = 0;
    }
}
...
void Display_closure(int states, int closure_ar[],
    ...
    for (i = 0; i < states; i++) {
        ...
        if (strcmp(&NFA_TABLE[i][symbols], "-") != 0) {
            ...
            // till closure gets completely saturated
            while (z != 100) {
        ...
        // print the e closure for every state of NFA
...
int new_states(struct DFA *dfa, char S[]) {
    int i;
    // To check if the current state is already being used as a DFA state or not in DFA transition table
    for (i = 0; i < last_index; i++) {
...
## OUTPUT 
![image](https://github.com/SudharsanamRK/19CS409-Compiler-Design-Lab/assets/115523484/23834d8d-7ef0-4753-9fb8-4e7cb70f7cc3)
